...
def framestep(cap, data_matrix):
    ...
    while curr_frame < frame_count:
        ret, frame = cap.read()
        if curr_frame % 10 == 0:
            print("  analyzing frame: ", i, " of ", frame_count)
...
def find_outliers(data_matrix, threshold=4):
...
def save_new_video(cap, path, output_directory, g_points, b_points, f_points):
    ...
    print(" saving video to", new_file_name)
...
def save_to_csv(good_points, path):
    ...
    print(" saving csv data to", new_file_name)
...
if __name__ == "__main__":
    ...
    #load the model
    net = cv2.dnn.readNetFromCaffe('Openpose/pose_deploy.prototxt', 'Openpose/pose_iter_584000.caffemodel')
